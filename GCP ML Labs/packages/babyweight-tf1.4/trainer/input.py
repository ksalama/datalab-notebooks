...
def process_features(features, hyper_params=None):
...
def dataset_input_fn(file_names_pattern,
    ...
    dataset = dataset.map(lambda features: get_features_target_tuple(features),
                          num_parallel_calls=num_threads)
    dataset = dataset.map(lambda features, target: (process_features(features, hyper_params), target),
                          num_parallel_calls=num_threads)
...
def load_feature_stats(hyper_params):
    ...
    In practice, the statistics of large dataset are computed prior to model training,
    ...
    in the --feature-stats argument. However, it can be a local path as well.
